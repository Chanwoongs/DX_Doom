...
bool ParticleSystemClass::Initialize(ID3D11Device * device, const WCHAR * textureFilename, 
	float deviationX, float deviationY, float deviationZ)
{
	bool result;
	...
	result = InitializeParticleSystem(deviationX, deviationY, deviationZ);
	if (!result)
	{
		return false;
	}
...
void ParticleSystemClass::SetParticleMode(int particleMode)
{
...
bool ParticleSystemClass::InitializeParticleSystem(float deviationX, float deviationY, float deviationZ)
{
	int i;
...
void ParticleSystemClass::EmitParticles(float frameTime)
{
	...
	if ((emitParticle == true) && (m_currentParticleCount < (m_maxParticles - 1)))
	{
		...
		else if (m_particleMode == 1)
		{
			m_particleList[index].red = red;
			m_particleList[index].green = 0.0f;
			m_particleList[index].blue = 0.0f;
		}
