...
bool SpriteClass::Initialize(ID3D11Device* device, int spriteWidth, int spriteHeight, const WCHAR* textureFilename)
{
	bool result;
...
bool SpriteClass::InitializeBuffers(ID3D11Device* device)
{
	...
	vertexBufferDesc.Usage = D3D11_USAGE_DYNAMIC; // 다이내믹하게 잡음. 사이즈를 줄일 수도 있기 때문에
...
bool SpriteClass::UpdateBuffers(ID3D11DeviceContext* deviceContext, float positionX, float positionY)
{
	...
	left = (float)positionX - (float)(m_spriteWidth / 2);
